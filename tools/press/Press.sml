...
local fun getLines (json_str:string) : line list =
      ...
      fun pp_cname cn =
          case cn of
            ...
            | "MLKIT [MLCOMP=mlkit-seq -no_gc -par -mlb-subdir P1]" => "par1"
            | "MLKIT [MLCOMP=mlkit-par -no_gc -par]" => "par"
            | _ =>
              if String.isPrefix "MPL" cn
              then "mpl"
              else if String.isPrefix "MLKIT [MLCOMP=mlkit-par" cn andalso String.isSubstring "-argo" cn
              then "abt"
              else if String.isPrefix "MLKIT [MLCOMP=mlkit-seq" cn andalso String.isSubstring "-argo" cn
              then "abt1"
              else cn
